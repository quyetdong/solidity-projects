...
async function filterEvents (callerContract) {
  ...
  if (!callerContract.events.ReceiveNewRequestIdEvent) return

  callerContract.events.ReceiveNewRequestIdEvent({ filter: { } }, async (err, event) => {
    if (err) console.error('Error on event', err)
    //console.log('* Received New Request event. requestId: ' + event.returnValues.id)
  })
}
